...
// Obtenir l'heure actuelle pour le fichier
	...
	if (file == NULL) {
		file = fopen("scores.txt", "w"); // CrC)e le fichier s'il n'existe pas
	}

	if (file != NULL) {
		// Cherche si le joueur existe dC)jC  dans le fichier
		...
		// Si le joueur est trouvC), met C  jour son score et la date
		if (trouve) {
			fclose(file);
			file = fopen("scores.txt", "r+"); // Ouvre C  nouveau en mode lecture/C)criture
			FILE *temp_file = fopen("temp.txt", "w"); // CrC)e un fichier temporaire pour la mise C  jour
			...
			while (fgets(ligne, sizeof(ligne), file)) {
				...
				if (sscanf(ligne, "Nom: %[^,], score: %d, date: %s", nom_lu, &score_lu, date_heure) == 3) {
					if (strcmp(nom_joueur, nom_lu) == 0) {
						fprintf(temp_file, "Nom: %s, score: %d, date: %s\n", nom_joueur, score, date_heure); // Mettre C  jour le score et la date
					} else {
...
void charger_score(char *nom_joueur, int *compteur) {
	...
	while (fgets(ligne, sizeof(ligne), file)) {
		...
		if (sscanf(ligne, "Nom: %[^,], score: %d, date: %s", nom_lu, &score_lu, date_lu) == 3) {
			// Si le nom correspond C  l'utilisateur, on charge son score et la date
			if (strcmp(nom_joueur, nom_lu) == 0) {
				*compteur = score_lu; // Charger le score existant
				printf("Bienvenue %s, votre score est : %d, enregistrC) le : %s\n", nom_joueur, *compteur, date_lu);
				trouve = 1;
				break; // Sortir de la boucle une fois le score trouvC)
			}
		}
	}
	fclose(file);

	// Si le joueur n'a pas de score enregistrC), initialiser C  0
	if (!trouve) {
		*compteur = 0;
		printf("Bienvenue %s, vous n'avez aucun enregistrement de score. Votre score a C)tC) initialisC)  C  0\n", nom_joueur);
	}
}
